...
{
    public class DroneMoveHandlerTests
    {
        ...
        {
            var droneMoveHandler = new DroneMoveHandler();

            droneMoveHandler.Process(_battlefieldContextMock.Object);

            _battlefieldContextMock.Verify(x => x.AddDroneCommand(It.IsAny<DroneMoveCommand>()), Times.Once);
        }
    }
}